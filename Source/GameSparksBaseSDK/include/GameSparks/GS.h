	...
	namespace Core
	{
		...
		class GS_API GS // this naming should be discussed, it was chosen because we need the static object with the name "GS"
		{
			public:
				...
				/*!
				 * unregisters the message listener registered for *MessageType* (if any).
				 *
				 * Note: you can also unregister a message listener by passing an empty functor (or nullptr) to
				 * SetMessageListener like so: SetMessageListener<MessageType>({})
				 *
				 * */
				template <typename MessageType>
				void ClearMessageListener()
				{
					SetMessageListener<MessageType>({});
				}

                /*! clears all message listeners currently registered. */
			...
			private:
				...
				friend class ::TestSerializeRequestQueue_Test_Test;
